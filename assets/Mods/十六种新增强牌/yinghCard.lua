...
function SMODS.INIT.JellyTarots18()
    sendDebugMessage("Loaded JellyTarots18~")
    local enhance_localization = {
        m_yingh = {
            name = "Black Soul Card",
                text = {
                    "{C:inactive}Hidden Enhancement Cards",
                    "After being scored, there is a {C:green}#1#/#2#{} chance",
                    "Generate a {C:spectral} soul",
                    "(There must be space)",
       },
       },
    }

    updateLocalizationJelly18(enhance_localization, "Enhanced")

    local enhancements = {
        m_yingh = {max = 500, order = 26, name = "yingh Card", set = "Enhanced", pos = {x=1,y=0}, effect = "yingh Card", label = "yingh Card", config = {extra=100}},
    }

    SMODS.Sprite:new("yingh", SMODS.findModByID("yinghCard").path, "DarkEnhancers.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "yingh")
...
function updateLocalizationJelly18(localizationTable, cardType)
...
function eval_card(card, context)
	if context.cardarea == G.play and not context.repetition_only and not card.debuff and card.ability.name == 'yingh Card' then
