...
function SMODS.INIT.JellyTarots7()
    sendDebugMessage("Loaded JellyTarots7~")
    local enhance_localization = {
        m_tanl = {
            name = "Greed",
                text = {
                    "{X:mult,C:white} X#1# {} Multiple",
                    "Get {C:money}$#2#{} when being scored",
       },
       },
    }

    updateLocalizationJelly7(enhance_localization, "Enhanced")

    local enhancements = {
        m_tanl = {max = 500, order = 15, name = "tanl Card", set = "Enhanced", pos = {x=0,y=5}, effect = "tanl Card", label = "tanl Card", config = {Xmult=1.2, p_dollars = 2}},
    }

    SMODS.Sprite:new("tanl", SMODS.findModByID("tanlCard").path, "Enhancers_JellyMod.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "tanl")
    local tanlan_tarot_def = {
        name = "monopoly",
        text = {
    ...
    local tanlan_tarot = SMODS.Tarot:new("The tanlan", "tanlan_tarot", {mod_conv = 'm_tanl', max_highlighted = 1}, { x = 9, y = 1 }, tanlan_tarot_def, 3, 1.0, "Suit Conversion", true, true)
    SMODS.Sprite:new("c_tanlan_tarot", SMODS.findModByID("tanlCard").path, "JoJoTarots.png", 71, 95, "asset_atli"):register();
    tanlan_tarot:register()
end

function updateLocalizationJelly7(localizationTable, cardType)
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    if _c.name == 'tanl Card' or _c.name == 'The tanlan' then
        customCard = true
    end
    ...
    elseif _c.set == 'Enhanced' then 
        ...
        if _c.name == 'tanl Card' then 
	    loc_vars = {_c.config.Xmult, _c.config.p_dollars}
	end
	localize{type = 'descriptions', key = _c.key, set = _c.set, nodes = desc_nodes, vars = loc_vars}
