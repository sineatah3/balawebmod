...
function SMODS.INIT.JellyTarots3()
    sendDebugMessage("Loaded JellyTarots3~")
        ...
        m_fulu = {
            name = "Fulu brand",
                text = {
                    "This card is",
                    "When left in hand",
                    "Will give {C:mult}+#1#{} multiplier",
       },
       },
    }

    updateLocalizationJelly3(enhance_localization, "Enhanced")

    local enhancements = {
        m_fulu = {max = 500, order = 12, name = "fulu Card", set = "Enhanced", pos = {x=3,y=5}, effect = "fulu Card", label = "fulu Card", config = {h_mult = 4}},
    }

    SMODS.Sprite:new("fulu", SMODS.findModByID("fuluCard").path, "Enhancers_JellyMod.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "fulu")
    local hunjun_tarot_def = {
        name = "Black Emperor",
        text = {
    ...
    local hunjun_tarot = SMODS.Tarot:new("The hunjun", "hunjun_tarot", {mod_conv = 'm_fulu', max_highlighted = 2}, { x = 5, y = 0 }, hunjun_tarot_def, 3, 1.0, "Suit Conversion", true, true)
    SMODS.Sprite:new("c_hunjun_tarot", SMODS.findModByID("fuluCard").path, "JoJoTarots.png", 71, 95, "asset_atli"):register();
    hunjun_tarot:register()
end

function updateLocalizationJelly3(localizationTable, cardType)
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    if _c.name == 'fulu Card' or _c.name == 'The hunjun' then
        customCard = true
    end
    ...
    elseif _c.set == 'Enhanced' then 
        ...
        if _c.name == 'fulu Card' then 
	    loc_vars = {_c.config.h_mult}
	end
	localize{type = 'descriptions', key = _c.key, set = _c.set, nodes = desc_nodes, vars = loc_vars}
