...
function SMODS.INIT.JellyTarots8()
    sendDebugMessage("Loaded JellyTarots8~")
    local enhance_localization = {
        m_bux = {
            name = "Bux Card",
                text = {
                    "{X:mult,C:white} X#3# {} Multiple",
                    "have{C:red}#1#/#2#{}probability",
                    "lose{C:money}$2{}",
       },
       },
    }

    updateLocalizationJelly8(enhance_localization, "Enhanced")

    local enhancements = {
        m_bux = {max = 500, order = 17, name = "bux Card", set = "Enhanced", pos = {x=5,y=5}, effect = "bux Card", label = "bux Card", config = {extra = 4, Xmult=2}},
    }

    SMODS.Sprite:new("bux", SMODS.findModByID("buxCard").path, "Enhancers_JellyMod.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "bux")
    local buxing_tarot_def = {
        name = "Desire",
        text = {
            "Enhanced {C:attention} 1",
                   "Select the card to become",
                   "{C:attention}Unlucky Card",
        }
    }

    local buxing_tarot = SMODS.Tarot:new("The buxing", "buxing_tarot", {mod_conv = 'm_bux', max_highlighted = 1}, { x = 4, y = 0 }, buxing_tarot_def, 3, 1.0, "Suit Conversion", true, true)
    SMODS.Sprite:new("c_buxing_tarot", SMODS.findModByID("buxCard").path, "RS-Tarots.png", 71, 95, "asset_atli"):register();
    buxing_tarot:register()
end

function updateLocalizationJelly8(localizationTable, cardType)
...
function eval_card(card, context)
	if context.cardarea == G.play and not context.repetition_only and not card.debuff and card.ability.name == 'bux Card' then
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    if _c.name == 'bux Card' or _c.name == 'The buxing' then
        customCard = true
    end
    ...
    elseif _c.set == 'Enhanced' then 
        ...
        if _c.name == 'bux Card' then 
	    loc_vars = {G.GAME.probabilities.normal, _c.config.extra, _c.config.Xmult}
	end
	localize{type = 'descriptions', key = _c.key, set = _c.set, nodes = desc_nodes, vars = loc_vars}
