...
function SMODS.INIT.JellyTarots9()
    sendDebugMessage("Loaded JellyTarots9~")
    local enhance_localization = {
        m_fuh = {
            name = "Corruption Card",
                text = {
                   "When being scored",
                   "There is a {C:green}#1#/#2#{} chance",
                   "Temporary {C:attention}+#3#{}Hand limit",
       },
       },
    }

    updateLocalizationJelly9(enhance_localization, "Enhanced")

    local enhancements = {
        m_fuh = {max = 500, order = 16, name = "fuh Card", set = "Enhanced", pos = {x=0,y=0}, effect = "fuh Card", label = "fuh Card", config = {extra = 4}},
    }

    SMODS.Sprite:new("fuh", SMODS.findModByID("fuhCard").path, "fuh Card.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "fuh")
    local fuhua_tarot_def = {
        name = "pollute",
        text = {
    ...
    local fuhua_tarot = SMODS.Tarot:new("The fuhua", "fuhua_tarot", {mod_conv = 'm_fuh', max_highlighted = 1}, { x = 0, y = 2 }, fuhua_tarot_def, 3, 1.0, "Suit Conversion", true, true)
    SMODS.Sprite:new("c_fuhua_tarot", SMODS.findModByID("fuhCard").path, "RS-Tarots.png", 71, 95, "asset_atli"):register();
    fuhua_tarot:register()
end

function updateLocalizationJelly9(localizationTable, cardType)
...
function eval_card(card, context)
	if context.cardarea == G.play and not context.repetition_only and not card.debuff and card.ability.name == 'fuh Card' then
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    if _c.name == 'fuh Card' or _c.name == 'The fuhua' then
        customCard = true
    end
