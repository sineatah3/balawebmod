...
function SMODS.INIT.JellyTarots14()
    sendDebugMessage("Loaded JellyTarots14~")
        ...
        m_anj = {
            name = "Dark Gold",
                text = {
                    ...
                    "This card is",
                    "When left in hand",
                    "Will give{X:mult,C:white} X#1# {} Multiplier",
                    "Still in hand at the end of the turn",
                    "You get{C:money}$5",
       },
       },
    }

    updateLocalizationJelly14(enhance_localization, "Enhanced")

    local enhancements = {
        m_anj = {max = 500, order = 22, name = "anj Card", set = "Enhanced", pos = {x=6,y=0}, effect = "anj Card", label = "anj Card", config = {h_x_mult=1.2, h_dollars = 5}},
    }

    SMODS.Sprite:new("anj", SMODS.findModByID("anjCard").path, "DarkEnhancers.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "anj")
...
function updateLocalizationJelly14(localizationTable, cardType)
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    elseif _c.set == 'Enhanced' then 
        ...
        if _c.name == 'anj Card' then 
	    loc_vars = {_c.config.h_x_mult, _c.config.h_dollars}
	end
	localize{type = 'descriptions', key = _c.key, set = _c.set, nodes = desc_nodes, vars = loc_vars}
