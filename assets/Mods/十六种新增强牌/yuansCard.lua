...
function SMODS.INIT.JellyTarots19()
    sendDebugMessage("Loaded JellyTarots19~")
    local enhance_localization = {
        m_yuans = {
            name = "Round Stone",
                text = {
                    "{C:inactive}Hidden enhancement card",
                    "When being scored",
                    "Reduce the current {C:attention} blind{}",
                    "Score Required {C:attention}5%{}",
       },
       },
    }

    updateLocalizationJelly19(enhance_localization, "Enhanced")

    local enhancements = {
        m_yuans = {max = 500, order = 27, name = "yuans Card", set = "Enhanced", pos = {x=1,y=5}, effect = "yuans Card", label = "yuans Card", config = {}},
    }

    SMODS.Sprite:new("yuans", SMODS.findModByID("yuansCard").path, "RS-Enhancers.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "yuans")
...
function updateLocalizationJelly19(localizationTable, cardType)
...
function eval_card(card, context)
	if context.cardarea == G.play and not context.repetition_only and not card.debuff and card.ability.name == 'yuans Card' then
