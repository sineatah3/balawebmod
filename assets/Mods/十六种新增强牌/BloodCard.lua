...
function SMODS.INIT.JellyTarots2()
    sendDebugMessage("Loaded JellyTarots2~")
    local enhance_localization = {
        m_blood = {
            name = "blood Card",
                text = {
                   "When being scored",
                   "have{C:green}1/#2#{}{C:red}fixed{}probability",
                    "temporary{C:blue}+#3#{}Number of Cards Played",
       },
       },
    }

    updateLocalizationJelly2(enhance_localization, "Enhanced")

    local enhancements = {
        m_blood = {max = 500, order = 11, name = "Blood Card", set = "Enhanced", pos = {x=0,y=0}, effect = "Blood Card", label = "Blood Card", config = {extra = 4}},
    }

    SMODS.Sprite:new("Blood", SMODS.findModByID("BloodCard").path, "Blood Card.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "Blood")
    local bloody_tarot_def = {
        name = "Killing",
        text = {
    ...
    local bloody_tarot = SMODS.Tarot:new("The bloody", "bloody_tarot", {mod_conv = 'm_blood', max_highlighted = 1}, { x = 3, y = 1 }, bloody_tarot_def, 3, 1.0, "Suit Conversion", true, true)
    SMODS.Sprite:new("c_bloody_tarot", SMODS.findModByID("BloodCard").path, "JoJoTarots.png", 71, 95, "asset_atli"):register();
    bloody_tarot:register()
end

function updateLocalizationJelly2(localizationTable, cardType)
...
function eval_card(card, context)
	if context.cardarea == G.play and not context.repetition_only and not card.debuff and card.ability.name == 'Blood Card' then
		if pseudorandom('sample') < 1/card.ability.extra then
			ease_hands_played(1)
		end
	end
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    if _c.name == 'Blood Card' or _c.name == 'The bloody' then
        customCard = true
    end
