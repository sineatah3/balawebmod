...
function SMODS.INIT.JellyTarots22()
    sendDebugMessage("Loaded JellyTarots22~")
        ...
        m_lvtie = {
            name = "Aluminum",
                text = {
                    ...
                    "and {X:mult,C:white} X#2# {} Multiplier",
       },
       },
    }

    updateLocalizationJelly22(enhance_localization, "Enhanced")

    local enhancements = {
        m_lvtie = {max = 500, order = 30, name = "lvtie Card", set = "Enhanced", pos = {x=6,y=1}, effect = "lvtie Card", label = "lvtie Card", config = {h_mult=6, h_x_mult = 2}},
    }

    SMODS.Sprite:new("lvtie", SMODS.findModByID("lvtieCard").path, "RS-Enhancers.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "lvtie")
...
function updateLocalizationJelly22(localizationTable, cardType)
...
function eval_card(card, context)
	if context.cardarea == G.play and not context.repetition_only and not card.debuff and card.ability.name == 'suilie Card' then
		if pseudorandom('sample') < G.GAME.probabilities.normal/card.ability.extra then
			...
			if G.GAME.hands[chosen_hand].level <= 1 then delay(1.5) else
				level_up_hand(nil, chosen_hand, nil, -1 * (1))
			end
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    elseif _c.set == 'Enhanced' then 
        ...
        if _c.name == 'lvtie Card' then 
	    loc_vars = {_c.config.h_mult, _c.config.h_x_mult}
	end
	localize{type = 'descriptions', key = _c.key, set = _c.set, nodes = desc_nodes, vars = loc_vars}
