...
function SMODS.INIT.JellyTarots16()
    sendDebugMessage("Loaded JellyTarots16~")
        ...
        m_caiyun = {
            name = "Fortune Card",
                text = {
                    ...
                    "{C:mult}+#3#{} Multiple",
                    "{C:green}#1#/#2#{}probability",
                    "get{C:money}$20",
       },
       },
    }

    updateLocalizationJelly16(enhance_localization, "Enhanced")

    local enhancements = {
        m_caiyun = {max = 500, order = 24, name = "caiyun Card", set = "Enhanced", pos = {x=4,y=1}, effect = "caiyun Card", label = "caiyun Card", config = {extra=10, mult=20}},
    }

    SMODS.Sprite:new("caiyun", SMODS.findModByID("caiyunCard").path, "Enhancedments2.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "caiyun")
...
function updateLocalizationJelly16(localizationTable, cardType)
...
function eval_card(card, context)
	if context.cardarea == G.play and not context.repetition_only and not card.debuff and card.ability.name == 'caiyun Card' then
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    elseif _c.set == 'Enhanced' then 
        ...
        if _c.name == 'caiyun Card' then 
	    loc_vars = {G.GAME.probabilities.normal, _c.config.extra, _c.config.mult}
	end
	localize{type = 'descriptions', key = _c.key, set = _c.set, nodes = desc_nodes, vars = loc_vars}
