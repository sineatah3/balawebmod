...
function SMODS.INIT.JellyTarots6()
    sendDebugMessage("Loaded JellyTarots6~")
    ...
    updateLocalizationJelly6(enhance_localization, "Enhanced")

    local enhancements = {
        m_yuanm = {max = 500, order = 15, name = "yuanm Card", set = "Enhanced", pos = {x=1,y=5}, effect = "yuanm Card", label = "yuanm Card", config = {mult=2, bonus=15}},
    }

    SMODS.Sprite:new("yuanm", SMODS.findModByID("yuanmCard").path, "Enhancers_JellyMod.png", 71, 95, "asset_atli")
    :register()
    addEnhancementsToPools(enhancements, "yuanm")
    local yuanmu_tarot_def = {
        name = "Moon Bud",
        text = {
            "Enhanced {C:attention} 2 sheets",
                   "Select the card to become",
                   "{C:attention}Wooden sign",
        }
    }

    local yuanmu_tarot = SMODS.Tarot:new("The yuanmu", "yuanmu_tarot", {mod_conv = 'm_yuanm', max_highlighted = 2}, { x = 8, y = 1 }, yuanmu_tarot_def, 3, 1.0, "Suit Conversion", true, true)
    SMODS.Sprite:new("c_yuanmu_tarot", SMODS.findModByID("yuanmCard").path, "JoJoTarots.png", 71, 95, "asset_atli"):register();
    yuanmu_tarot:register()
end

function updateLocalizationJelly6(localizationTable, cardType)
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    if _c.name == 'yuanm Card' or _c.name == 'The yuanmu' then
        customCard = true
    end
    ...
    elseif _c.set == 'Enhanced' then 
        ...
        if _c.name == 'yuanm Card' then 
	    loc_vars = {_c.config.mult, _c.config.bonus}
	end
	localize{type = 'descriptions', key = _c.key, set = _c.set, nodes = desc_nodes, vars = loc_vars}
